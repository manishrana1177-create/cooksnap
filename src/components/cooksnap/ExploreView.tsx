...
export default function ExploreView() {
  ...
  // Auto-generate recipes when pantry has items (only once)
  useEffect(() => {
    if (!hasLoadedPantry || hasAutoGenerated.current) return

    if (pantryItems.length > 0) {
      hasAutoGenerated.current = true
      generateRecipesFromPantry(pantryItems)
    } else {
      setRecipes(sampleExploreRecipes)
    }
  }, [hasLoadedPantry, pantryItems])

  const generateRecipesFromPantry = async (items: { name: string; category: string }[]) => {
    if (items.length === 0) {
      setRecipes(sampleExploreRecipes)
      return
    }
    ...
    try {
      const ingredientNames = items.map((p) => p.name)
      ...
      setRecipes(sampleExploreRecipes)
      toast({
        title: isTimeout ? 'AI is taking too long' : 'Using sample recipes',
        description: isTimeout
          ? 'Recipe generation timed out. Showing popular recipes instead.'
          : 'Could not generate custom recipes. Showing popular recipes instead.',
      })
    } finally {
      ...
      <div className="flex-1 px-4 py-4 space-y-4 pb-24">
        ...
        {isGenerating && (
          <div className="flex flex-col items-center justify-center py-16">
            ...
            <p className="text-xs text-gray-300 mt-3">This may take up to 60 seconds</p>
          </div>
        )}
